...
def new_diagram(driver, plant_code, diagram_name, template_path, codes_to_replace, diagram_type):
    ...
    replace_datapoints_reutrn_value = replace_datapoints(driver, plant_code, diagram_type, codes_to_replace)
...
def search_for_plant(driver, plant_code):
    ...
    for i in range(2):
        ...
        if len(panel) > 0:
            ...
            print(len(plant_button))
            #print("plant button text: " + plant_button[1].text)
...
def replace_datapoints(driver, plant_code, diagram_type, codes_to_replace):
    ...
    while True:
        ...
        if len(current_datapoint) != 0:
            ...
            break
    print("diagram datapoint list: ", diagram_datapoint_list)
    print("datapoint list: ", datapoint_list)
    datapoints_to_delete = []
    for diagram_datapoint in diagram_datapoint_list:
        existing = False
        datapoint_found = False
        for datapoint in datapoint_list:
            print("datapoint: ", datapoint[0])
            print("diagram_datapoint: ", plant_code, diagram_datapoint[0])
